...
if __name__ == '__main__':
    ...
    for i in range(test_number):
        ...
        intervals: List[List[int]] = convert_params(data[i * one_test_number + 0], 'List[List[int]]')
        newInterval: List[int] = convert_params(data[i * one_test_number + 1], 'List[int]')
        real_res: List[List[int]] = convert_params(data[i * one_test_number + 2], 'List[List[int]]')

        my_res: List[List[int]] = sol.insert(intervals, newInterval)
