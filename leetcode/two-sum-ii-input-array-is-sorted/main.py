...
if __name__ == '__main__':
    ...
    for i in range(test_number):
        ...
        numbers: List[int] = convert_params(data[i * one_test_number + 0], 'List[int]')
        target: int = convert_params(data[i * one_test_number + 1], 'int')
        real_res: List[int] = convert_params(data[i * one_test_number + 2], 'List[int]')

        my_res: List[int] = sol.twoSum(numbers, target)

        check_result = compare_result(f"{i + 1}", my_res, real_res, 'List[int]')
        if not check_result:
            fail_test += 1
