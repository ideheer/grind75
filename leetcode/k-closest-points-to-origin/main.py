...
if __name__ == '__main__':
    ...
    for i in range(test_number):
        ...
        points: List[List[int]] = convert_params(data[i * one_test_number + 0], 'List[List[int]]')
        k: int = convert_params(data[i * one_test_number + 1], 'int')
        real_res: List[List[int]] = convert_params(data[i * one_test_number + 2], 'List[List[int]]')

        my_res: List[List[int]] = sol.kClosest(points, k)
