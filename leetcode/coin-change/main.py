...
if __name__ == '__main__':
    ...
    for i in range(test_number):
        ...
        coins: List[int] = convert_params(data[i * one_test_number + 0], 'List[int]')
        amount: int = convert_params(data[i * one_test_number + 1], 'int')
        real_res: int = convert_params(data[i * one_test_number + 2], 'int')

        my_res: int = sol.coinChange(coins, amount)
