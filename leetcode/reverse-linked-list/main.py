...
if __name__ == '__main__':
    ...
    for i in range(test_number):
        ...
        head: ListNode = convert_params(data[i * one_test_number + 0], 'ListNode')
        real_res: ListNode = convert_params(data[i * one_test_number + 1], 'ListNode')

        my_res: ListNode = sol.reverseList(head)

        check_result = compare_result(f"{i + 1}", my_res, real_res, 'ListNode')
        if not check_result:
            fail_test += 1
