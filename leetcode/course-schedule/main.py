...
if __name__ == '__main__':
    ...
    for i in range(test_number):
        ...
        numCourses: int = convert_params(data[i * one_test_number + 0], 'int')
        prerequisites: List[List[int]] = convert_params(data[i * one_test_number + 1], 'List[List[int]]')
        real_res: bool = convert_params(data[i * one_test_number + 2], 'bool')

        my_res: bool = sol.canFinish(numCourses, prerequisites)

        check_result = compare_result(f"{i + 1}", my_res, real_res, 'bool')
        if not check_result:
            fail_test += 1
